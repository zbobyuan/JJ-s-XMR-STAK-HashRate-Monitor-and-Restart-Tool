...
do {
  ...
  } Else {     $slackUsername = $env:COMPUTERNAME       } 
    ...
    if (-not (Test-IsAdmin))
    {
      ...
      if ($scriptPath -replace '"' -match '.ps1$'){
        ...
        {    
          ...
          log-Write 'Failed to elevate to administrator' -fore Red -notification
          EXIT
        }
      } else {
        log-Write 'You need to run this as admin for xmrSTAK tomine efficent blocks' -fore Red -notification
        EXIT
        }
  ...
  function Check-Network {
    ...
    while (-not ($NetStatus) )
        ...
        if (-not($NetStatus)) {
            log-Write "Connection down: $($timer.Elapsed.ToString("hh\:mm\:ss"))" -fore red -notification
  ...
  function reset-VideoCard  {
    ...
    log-Write -logstring 'Resetting Video Card(s)...' -fore White -notification
    ...
    if ($CardResetEnabled -eq 'True'){
      ...
      log-Write -logstring "$vCTR Video Card(s) Reset" -fore yellow -notification
    } else {
      log-Write -logstring 'Card reset bypassed' -fore red -notification
    }
  }

  Function Pause-Then-Exit {
    log-Write -logstring 'Pause for user and exit called' -fore Red -notification
    Pause
    EXIT
  }
  ...
  Function refreshSTAK {
    ...
    {
      Clear-Host
      log-Write -logstring 'Restarting in 10 seconds - Lost connectivity to STAK' -fore red -notification
      $script:currHash = 0
  ...
  Function Run-Tools {  
    ...
    foreach ($item in $app)
    {
      ...
      {
        log-Write -logstring "Starting $item" -fore green -notification
        If ($prog[1]) {
  ...
  function start-Mining {
    ...
    If (Test-Path ($STAK))
    {
      ...
      If ($STAKcmdline)     {
        ...
        log-Write "$script:STAKexe NOT FOUND.. EXITING" -fore Red -notification
    ...
    if (-not($stakPROC)) {
        write-host "$prog"
        log-Write 'stak exited abnormally, Run manually and check output' -fore red -notification
        Pause-Then-Exit
    }
  }
  ...
  Function test-cards(){
    ...
    { 
      ...
      $flag = 'True'
        ...
        log-Write -logstring 'Driver in error state, Resetting' -fore red -notification
        reset-VideoCard
        ...
        if (Supported-Cards-OK)
        {
            ...
            {
                log-Write -logstring "Driver in error state, Restart-Computer in $rebootTimeout" -fore red -notification
                Start-Sleep -Seconds $rebootTimeout
                Restart-Computer -Force 
                EXIT
            } else {
                log-Write -logstring 'Reboot not enabled' -fore red -notification
                Pause-Then-Exit
            }
        } # End of device test 
    } # End of driver error 
  ...
  Function quickcheckSTAK {  
    ...
    If (-not($script:STAKisup))
    {
      ...
      if ($ResetCardOnStartup -eq 'True') {reset-VideoCard -force $true}
      test-cards
      disable_crossfire
  ...
  Function chk-STAK {
    ...
    {
      Clear-Host
      log-Write -logstring '!! Timed out waiting for STAK HTTP daemon to start !!' -fore red -notification
      ...
      if (check-Process -exe $script:STAKexe) {
        ...
        log-Write -fore red -logstring "Abnormal Stak process seems to have hung on strartup or your minhashrate $minhashrate is too low, attempt $startattempt" -notification
        if ( ($startattempt -ge $STAKMaxStartAttempts ) -and ($STAKMaxStartAttempts -gt 0)) {
            log-write -fore red "Restarting computer in 10 seconds" -notification
            start-sleep -s 10
            Restart-Computer -Force 
        } elseif (($startattempt -ge $STAKMaxStartAttempts ) -and ($STAKMaxStartAttempts -eq 0) ) {
            log-write -froe red "Reboot disabled, stopping here, please investigate STAK startup" -notification
            Pause-Then-Exit
        } else {
            call-self
            } 
    ...
    {
      Clear-Host
      log-Write -logstring 'Unknown failure starting STAK (Daemon failed to start?)' -fore red -notification
      Pause-Then-Exit
    }
  }
  ...
  function starting-Hash{
    ...
    If (!$currTestHash)  {
      Clear-Host
      log-Write -logstring 'Could not get hashrate... restarting in 3 seconds' -fore Red -notification
    ...
    log-Write -logstring "Starting Hashrate: $script:maxhash H/s	Drop Target Hashrate: $script:rTarget H/s" -fore Green -notification
  }
  ...
  function current-Hash{
    ...
    If (($flag -eq 'True') -And ($script:currHash -lt $minhashrate ))
    {
      ...
      log-Write -logstring "Restarting in 10 seconds after $tFormat - Hash rate dropped from $script:maxhash H/s to $script:currHash H/s" -fore Red -notification
      Start-Sleep -Seconds 10
      $script:STAKisup = $false
      kill-Process -STAKexe ($STAKexe)
      call-self 
  ...
  function lowratecheck {
    ...
    log-Write -logstring "Low hash rate check triggered: $script:currHash " -fore Red
    $flag = 'False'
    ...
    {     
      $tFormat =  get-RunTime -sec ($runTime)
      log-Write -logstring "Restarting Script after $tFormat - Hash rate $script:currHash H/s less than set minimum $ratetocheck H/s" -fore red -notification
      kill-Process -STAKexe ($STAKexe)
      $script:STAKisup = $false
    ...
    try
    {
      ...
      if ($stakPROC) {
        ...
        if (!$stakPROC.HasExited) {
          ...
          log-Write -logstring "Failed to kill $prog" -fore 0 -notification
          Pause-Then-Exit
  ...
  do {
    ...
    if ($initalRun)
    {
      log-Write -fore White -linefeed -logstring 'Starting the Hash Monitor Script...' -notification
      $initalRun = $false
      } 
      Else 
      { log-Write -logstring '============= Loop Started =============' -fore Green -notification }
    ...
    # Setup for next run
    log-Write "Restart triggered`n`n" -fore Red -notification
    $running=$true
